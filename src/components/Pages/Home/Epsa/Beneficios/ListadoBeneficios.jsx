...
export const ListadoBeneficios = () => {
  ...
  const traerBeneficios = () => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/beneficios/beneficiospaginador?size=6&page=${pagina}`).then((response) => {
      setBeneficios(response.data.content)
    })
  }
  ...
  const actualizar = (prop) => {
    navigate("/Beneficio/Act/" + prop)
  }

  const eliminar = (prop) => {
 
    axios.delete("https://api.cruzpet.com:8443/v1.0/beneficios/eliminar/" + prop).then(() => {
        traerBeneficios()
    })
                ...
                {beneficios.map((response) =>
                  <tbody>
                    <tr>
                      ...
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => actualizar(response.idBeneficio)} className="text-dark-purple hover:text-blue-900">
                          Editar
                        </a>
                      </td>
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => eliminar(response.idBeneficio)} className="text-dark-purple hover:text-blue-900">
                          Eliminar
                        </a>
                      </td>
                    </tr>

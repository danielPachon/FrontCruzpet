...
export function ListadoRazas() {
  ...
  const traerRazas = () => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/razas/razaspaginador?size=6&page=${pagina}`).then((response) => {
      setRazas(response.data.content)
    })
  }
  ...
  const eliminarRaza = (prop) => {

    axios.delete("https://api.cruzpet.com:8443/v1.0/razas/eliminar/" + prop).then(() => {
      traerRazas()
    })
                ...
                {razas.map((response) =>
                  <tbody>
                    <tr>
                      ...
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => navigate("/Razas/Actualizar/" + response.idRaza)} className="text-dark-purple hover:text-blue-900">
                          Editar
                        </a>
                      </td>

                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => eliminarRaza(response.idRaza)} className="text-dark-purple hover:text-blue-900">
                          Eliminar
                        </a>
                      </td>
                    </tr>

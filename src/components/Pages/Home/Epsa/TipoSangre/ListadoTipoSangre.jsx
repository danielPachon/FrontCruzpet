...
export const ListadoTipoSangre = () => {
    ...
    const traerTiposSangre = () => {
      axios.get(`https://api.cruzpet.com:8443/v1.0/tipossangre/tiposangrepaginador?size=6&page=${pagina}`).then((response) => {
        setTiposSangre(response.data.content)
      })
    }
    ...
    const actualizar = (prop) => {
      navigate("/TiposSangre/Editar/" + prop)
    }
  
    const eliminar = (prop) => {
 
        axios.delete("https://api.cruzpet.com:8443/v1.0/tipossangre/eliminar/" + prop).then(() => {
            traerTiposSangre()
        })
                      ...
                      <tr>
                        <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                          <div className="flex items-center">
                            ...
                            <div className="ml-3">
                              <p className="text-gray-900 whitespace-no-wrap">
                                {response.tipoMascotaEntity.tipoMascota}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                          <a onClick={() => actualizar(response.idTipoSangre)} className="text-dark-purple hover:text-blue-900">
                            Editar
                          </a>
                        </td>
                        <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                          <a onClick={() => eliminar(response.idTipoSangre)} className="text-dark-purple hover:text-blue-900">
                            Eliminar
                          </a>
                        </td>
                      </tr>

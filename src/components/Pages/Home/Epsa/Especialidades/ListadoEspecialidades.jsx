...
export const ListadoEspecialidades = () => {
  ...
  const traerEspecialidades = () => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/especialidades/especialidadpaginador?size=6&page=${pagina}`).then((response) => {
  ...
  const actualizar = (prop) => {
    navigate("/Especialidad/Act/" + prop)
  }

  const eliminar = (prop) => {
 
    axios.delete("https://api.cruzpet.com:8443/v1.0/especialidades/eliminar/" + prop).then(() => {
        traerEspecialidades()
    })
                ...
                {especialidades.map((response) =>
                  <tbody>
                    <tr>
                      ...
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => actualizar(response.idEspecialidad)} className="text-dark-purple hover:text-blue-900">
                          Editar
                        </a>
                      </td>
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => eliminar(response.idEspecialidad)} className="text-dark-purple hover:text-blue-900">
                          Eliminar
                        </a>
                      </td>
                    </tr>

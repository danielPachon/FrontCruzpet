...
export function ListadoDirecciones() {
  ...
  const traerDirecciones = () => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/direcciones/direccionespaginador?size=6&page=${pagina}`).then((response) => {
      setDirecciones(response.data.content)
      for (let i = 0; i < response.data.content.length; i++) {

        setNombreBarrio(response.data.content[i].barrios.nombreBarrio)
        setNombreCiudad(response.data.content[i].barrios.ciudadOrigen.nombreCiudad)
        setNombreDepartamento(response.data.content[i].barrios.ciudadOrigen.departamentoOrigen.nombreDepartamento)
  ...
  const actualizar = (props) => {
    ...
    navigate("/Direcciones/" + props)

  }

  const buscador = async (event) => {
    await axios.get(`https://api.cruzpet.com:8443/v1.0/direcciones/direccionespaginadorpalabra?size=6&page=${pagina}&palabra=${event.target.value}`, {
    }).then((response) => {
          ...
          <div className="-mx-4 sm:-mx-8 md:px-4 sm:px-8 overflow-x-auto">
            <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
              <table className="min-w-full leading-normal">
                ...
                {direcciones.map((response, index) =>
                  <tbody>
                    <tr>

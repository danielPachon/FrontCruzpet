...
export function ListadoCiudades() {
  ...
  const traerCiudades = () => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/ciudades/ciudadespaginador?size=6&page=${pagina}`).then((response) => {
      setCiudades(response.data.content)
    })
  }
  ...
  const actualizar = (props) => {
    ...
    navigate("/Ciudades/" + props)

  }
  const buscador = async (event) => {
    await axios.get(`https://api.cruzpet.com:8443/v1.0/ciudades/ciudadespaginadorpalabra?size=6&page=${pagina}&palabra=${event.target.value}`, {
    }).then((response) => {
                ...
                {ciudades.map((response) =>
                  <tbody>
                    <tr>
                      ...
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <p className="text-gray-900 whitespace-no-wrap">
                          {response.departamentoOrigen.nombreDepartamento}
                        </p>
                      </td>
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => actualizar(response.idCiudad)} className="text-dark-purple hover:text-blue-900">
                          Editar
                        </a>
                      </td>
                    </tr>

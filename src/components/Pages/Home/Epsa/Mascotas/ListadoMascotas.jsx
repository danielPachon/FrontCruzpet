...
export const ListadoMascotas = () => {
    ...
    if (textoInput == "") {
      ...
      switch (textoLista) {
        case 'identificacion cliente':
          traerMascotasCliente(textoInput)
          break
  ...
  const traerMascotas = () => {
    axios.get("https://api.cruzpet.com:8443/v1.0/mascotas/").then((response) => {
      setMascotas(response.data)
      let arregloUsuarioMascota = []
      let arregloRaza = []
      let arregloGenero = []
      response.data.forEach((responses) => {
  ...
  const traerMascotasCliente = (props) => {
    axios.get(`https://api.cruzpet.com:8443/v1.0/mascotas/mostrarmascotastepalabra?size=6&page=0&palabra=${props}`).then((response) => {
      if (props == "") {
        traerMascotas()
      } else {
        console.log(props)
                ...
                {mascotas.map((response, index) =>
                  <tbody>
                    <tr>
                      ...
                      <td className="px-5 py-4 border-b border-gray-200 bg-white text-sm">
                        <a onClick={() => actualizar(response.numeroIdentidad)} href="#" className="text-dark-purple hover:text-blue-900">
                          Editar
                        </a>
                      </td>
                    </tr>
